"""Provide filters for querying close approaches & limit generated results.
...
class AttributeFilter:
    ...
    def __init__(self, op, value):
        ...
        This attribute is created from an binary predicate and a reference
        value.
    ...
    def __repr__(self):
        """Return object representation for system."""
        return (f"{self.__class__.__name__}(op"
                f"=operator.{self.op.__name__}, value={self.value})")
...
class DiameterFilter(AttributeFilter):
    ...
    @classmethod
    def get(cls, approach):
        """Given function overrides its superclass function."""
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    customfilterobject = []
    if (date is not None):
        df = DateFilter(operator.eq, date)
        customfilterobject.append(df)
    if (start_date is not None):
        sd = DateFilter(operator.ge, start_date)
        customfilterobject.append(sd)
    if (end_date is not None):
        ed = DateFilter(operator.le, end_date)
        customfilterobject.append(ed)
    if (distance_min is not None):
        min_d = DistanceFilter(operator.ge, distance_min)
        customfilterobject.append(min_d)
    if (distance_max is not None):
        max_d = DistanceFilter(operator.le, distance_max)
        customfilterobject.append(max_d)
    if (velocity_min is not None):
        v_min = VelocityFilter(operator.ge, velocity_min)
        customfilterobject.append(v_min)
    if (velocity_max is not None):
        v_max = VelocityFilter(operator.le, velocity_max)
        customfilterobject.append(v_max)
    if (diameter_min is not None):
        d_min = DiameterFilter(operator.ge, diameter_min)
        customfilterobject.append(d_min)
    if (diameter_max is not None):
        d_max = DiameterFilter(operator.le, diameter_max)
        customfilterobject.append(d_max)
    if (hazardous is not None):
        h = HazardousFilter(operator.eq, hazardous)
        customfilterobject.append(h)
    return customfilterobject
...
def limit(iterator, n=None):
    ...
    """
    if n == 0:
        return itertools.islice(iterator, None)
    elif n is None:
        return itertools.islice(iterator, None)
    else:
